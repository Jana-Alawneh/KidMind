...
import { ArrowLeft, Brain, Clock, Pause, Play, Square, Target, } from "lucide-react";
...
import CustomGamePlayer from "./CustomGamePlayer";
...
const SessionPlayer = () => {
            ...
            try {
                ...
                if (data.started_at &&
                    ...
                    if (Number.isFinite(startedAt)) {
                        startingSeconds =
                            Math.max(startingSeconds, Math.floor((Date.now() -
                                startedAt) / 1000));
                    }
                }
    ...
    const currentGame = games.find((game) => game.status ===
        "In Progress" ||
        game.status === "Paused") || null;
    const pendingGame = games.find((game) => game.status === "Pending") || null;
    ...
    const handleEndSession = async () => {
        ...
        const confirmed = window.confirm("Are you sure you want to end this session? The session status will become Ended.");
        if (!confirmed) {
            return;
        }
        try {
    ...
    const submitGameResult = async (gameId, gameResult) => {
        if (!session ||
            gameSaving) {
            return;
        }
        try {
            ...
            setGameSaveError(saveError?.response?.data?.message ||
                saveError?.message ||
    ...
    const renderFinishedSession = () => {
        if (session?.status ===
            ...
            <div>
              ...
              <button type="button" onClick={handleViewReport} className="
                  mt-7
                  bg-[#7B6EF6]
                  text-white
                  px-7
    ...
    const renderBetweenGames = () => {
        return (<div className="
          ...
          <div className="
            ...
            {lastFinishedGame && (<div className="
                  grid
                  grid-cols-3
                  gap-3
                  mt-7
    ...
    const renderSelectedGame = () => {
        ...
        if (Number.isInteger(customGameId) &&
            customGameId > 0) {
            return (<CustomGamePlayer key={`${currentGame.id}-${customGameId}`} gameId={customGameId} embedded paused={!running ||
                    actionLoading} onComplete={handleGameComplete}/>);
        }
        const gameName = currentGame.game_name
            ?.trim()
            .toLowerCase();
        if (gameName === "memory match") {
            return (<MemoryMatch key={currentGame.id} embedded difficulty={currentGame.difficulty} paused={!running ||
                    actionLoading} onComplete={handleGameComplete}/>);
        }
        if (gameName === "focus finder") {
            return (<FocusFinder key={currentGame.id} embedded difficulty={currentGame.difficulty} paused={!running ||
                    actionLoading} onComplete={handleGameComplete}/>);
        }
        if (gameName === "puzzle path") {
            return (<PuzzlePath key={currentGame.id} embedded difficulty={currentGame.difficulty} paused={!running ||
                    actionLoading} onComplete={handleGameComplete}/>);
        }
        if (gameName ===
            "reading adventure") {
            return (<ReadingAdventure key={currentGame.id} embedded difficulty={currentGame.difficulty} paused={!running ||
                    actionLoading} onComplete={handleGameComplete}/>);
        }
        if (gameName ===
            "quick match") {
            return (<QuickMatch key={currentGame.id} embedded difficulty={currentGame.difficulty} paused={!running ||
                    actionLoading} onComplete={handleGameComplete}/>);
        }
        return (<div className="
          min-h-[300px]
        ...
        {!loading &&
            ...
            <div className="
              ...
              {games.length > 0 && (<div className="
                  ...
                  {games.map((game, index) => (<div key={game.id} className="
                          border
                          rounded-2xl
                          px-4
                          py-3
                        ...
                        <span className={`
                            ...
                            text-xs
                            font-semibold
                            ${getGameStatusClasses(game.status)}
                          `}>
                          {game.status}
                        </span>
              ...
              {gameSaveError && (<div className="
                  ...
                  <button type="button" onClick={handleRetrySave} disabled={gameSaving} className="
                      bg-red-600
                      text-white
                      px-5
                      py-2
              ...
              {!isFinished && (<div className="
                  ...
                  <button type="button" onClick={handlePauseResume} disabled={actionLoading ||
                    gameSaving} title={running
                    ? "Pause Session"
                    : "Resume Session"} className="
                      w-14
                      h-14
                      rounded-2xl
                      bg-[#7B6EF6]
                  ...
                  <button type="button" onClick={handleEndSession} disabled={actionLoading ||
                    gameSaving} title="End Session" className="
                      w-14
                      h-14
                      rounded-2xl
                      bg-red-100
